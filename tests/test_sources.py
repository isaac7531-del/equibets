...
class EventSourceTests(unittest.TestCase):
    ...
    def test_event_level_lookup_covers_declared_domestic_and_international_levels(self):
        registry = load_event_source_registry()

        for domestic_level in registry.coverage_targets.domestic_event_levels:
            with self.subTest(domestic_level=domestic_level):
                source_ids = [source.id for source in sources_for_event_level(domestic_level)]
                self.assertIn("global_national_federations", source_ids)

        for international_level in registry.coverage_targets.international_event_levels:
            with self.subTest(international_level=international_level):
                source_ids = [source.id for source in sources_for_event_level(international_level)]
                self.assertIn("data_fei", source_ids)

    def test_event_level_lookup_normalizes_live_fei_labels(self):
