...
class EventSourceTests(unittest.TestCase):
    ...
    def test_all_declared_countries_and_levels_are_queryable(self):
        registry = load_event_source_registry()

        for country in registry.coverage_targets.countries:
            with self.subTest(country=country):
                source_ids = [source.id for source in sources_for_country(country)]
                self.assertEqual(source_ids[0], registry.primary_source_id)
                self.assertIn("global_national_federations", source_ids)
        ...
        for level in registry.coverage_targets.national_and_regional_levels:
            with self.subTest(level=level):
                ...
                self.assertNotIn(registry.primary_source_id, source_ids)
                self.assertIn("global_national_federations", source_ids)

        for level in registry.coverage_targets.fei_levels:
            with self.subTest(level=level):
                source_ids = [source.id for source in sources_for_event_level(level)]
                self.assertEqual(source_ids, [registry.primary_source_id])
