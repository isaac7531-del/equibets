...
class EventSourceTests(unittest.TestCase):
    ...
    def test_country_lookup_includes_all_country_national_backfill(self):
        source_ids = [source.id for source in sources_for_country("BRA", level="Introductory")]

        self.assertEqual(source_ids[0], "data_fei")
        self.assertIn("global_national_federations", source_ids)

    def test_country_lookup_includes_specific_priority_source(self):
