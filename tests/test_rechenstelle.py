...
BURGHLEY_LIVE_SANGER_HTML = """
<html>
  <head><title>LeaderBoard · Burghley 2026 · Defender Burghley CCI5*-L</title></head>
  <body>
    <p class="lastupdate">Last Update: Sep  3 2026 11:16AM</p>
    <table>
      <thead>
        <tr>
          <th>Start Time Dressage/ Rank</th><th>No.</th><th>Rider</th><th>&nbsp;</th><th>Horse</th>
...
BURGHLEY_LIVE_THURSDAY_AFTERNOON_HTML = """
<html>
  <head><title>LeaderBoard · Burghley 2026 · Defender Burghley CCI5*-L</title></head>
  <body>
    <p class="lastupdate">Last Update: Sep  3 2026  3:01PM</p>
    <table>
      <thead>
        <tr>
          <th>Start Time Dressage/ Rank</th><th>No.</th><th>Rider</th><th>&nbsp;</th><th>Horse</th>
      ...
      <tbody>
        ...
        <tr class="parent0">
          <td><strong>3.</strong></td>
          <td>18</td>
          <td class="riderCell"><span class="riderName">Max WARBURTON</span></td>
          <td><img src="../../../../flags/GBR.PNG" alt="GBR"></td>
          <td class="horseCell"><span class="horseName">Deerpairc Revelry</span></td>
          <td>537,0</td>
          <td>68,85</td>
          <td>31,2</td>
          <td>3.</td>
        ...
        <tr class="parent0">
          <td><strong>9.</strong></td>
          <td>19</td>
          <td class="riderCell"><span class="riderName">Tayla MASON</span></td>
          <td><img src="../../../../flags/NZL.PNG" alt="NZL"></td>
          <td class="horseCell"><span class="horseName">Centennial</span></td>
          <td>497,0</td>
          <td>63,72</td>
          <td>36,2</td>
          <td>9.</td>
...
BURGHLEY_LIVE_FRIDAY_FIRST_RIDE_HTML = """
<html>
  <head><title>LeaderBoard · Burghley 2026 · Defender Burghley CCI5*-L</title></head>
  <body>
    <p class="lastupdate">Last Update: Sep  4 2026 10:03AM</p>
    <table>
      <thead>
        <tr>
          <th>Start Time Dressage/ Rank</th><th>No.</th><th>Rider</th><th>&nbsp;</th><th>Horse</th>
...
BURGHLEY_LIVE_FRIDAY_SECOND_RIDE_HTML = """
<html>
  <head><title>LeaderBoard · Burghley 2026 · Defender Burghley CCI5*-L</title></head>
  <body>
    <p class="lastupdate">Last Update: Sep  4 2026 10:14AM</p>
    <table>
      <thead>
        <tr>
          <th>Start Time Dressage/ Rank</th><th>No.</th><th>Rider</th><th>&nbsp;</th><th>Horse</th>
...
BURGHLEY_LIVE_FRIDAY_EVENING_HTML = """
<html>
  <head><title>LeaderBoard · Burghley 2026 · Defender Burghley CCI5*-L</title></head>
  <body>
    <p class="lastupdate">Last Update: Sep  4 2026  4:23PM</p>
    <table>
      <thead>
        <tr>
...
BURGHLEY_LIVE_FINAL_HTML = """
<html>
  <head><title>LeaderBoard · Burghley 2026 · Defender Burghley CCI5*-L</title></head>
  <body>
    <p class="lastupdate">Last Update: Sep  6 2026  3:25PM</p>
    <table>
      <thead>
        <tr>
          <th>Rank</th><th>No.</th><th>Rider</th><th>&nbsp;</th><th>Horse</th>
...
<html>
  <head><title>LeaderBoard · Burghley 2026 · Defender Burghley CCI5*-L</title></head>
  <body>
    <p class="lastupdate">Last Update: Sep  3 2026  2:04PM</p>
    <table>
      <thead>
        <tr>
          <th>Start Time Dressage/ Rank</th><th>No.</th><th>Rider</th><th>&nbsp;</th><th>Horse</th>
...
class RechenstelleTests(unittest.TestCase):
    ...
    def test_burghley_sanger_dressage_inserts_fourth(self):
        board = RechenstelleBoard(
            url="https://live.rechenstelle.de/2026/burghley/leaderboard01.html",
            event_name="Burghley · CCI5*-L",
            level="CCI5*-L",
        ...
        results = parse_leaderboard_results(BURGHLEY_LIVE_SANGER_HTML, board=board)
        self.assertEqual(len(results), 3)
        ...
        self.assertEqual(
            ...
            [
                ("Global Quest", 32.1),
                ("Redfield Fyre", 38.2),
                ("Ballyneety Silver Service", 38.3),
            ],
        )
        sanger = next(result for result in results if result.horse_name == "Redfield Fyre")
        self.assertEqual(sanger.rider_name, "Cassie SANGER (USA)")
    ...
    def test_burghley_bragg_afternoon_dressage_inserts_sixth(self):
        board = RechenstelleBoard(
            url="https://live.rechenstelle.de/2026/burghley/leaderboard01.html",
            event_name="Burghley · CCI5*-L",
            level="CCI5*-L",
        ...
        results = parse_leaderboard_results(BURGHLEY_LIVE_BRAGG_HTML, board=board)
        self.assertEqual(len(results), 2)
        ordered = sorted(results, key=lambda result: result.finishing_score)
        self.assertEqual(
            [(result.horse_name, result.finishing_score) for result in ordered],
        ...
        bragg = ordered[1]
        self.assertEqual(bragg.rider_name, "Alexander BRAGG (GBR)")
        self.assertEqual(bragg.dressage_score, 35.4)
        self.assertEqual(bragg.show_jumping_penalties, 0.0)
        self.assertEqual(bragg.cross_country_jump_penalties, 0.0)
        self.assertEqual(bragg.cross_country_time_penalties, 0.0)
        self.assertNotIn("Centennial", {result.horse_name for result in results})

    def test_burghley_thursday_afternoon_dressage_adds_minder_lead(self):
        board = RechenstelleBoard(
            url="https://live.rechenstelle.de/2026/burghley/leaderboard01.html",
            event_name="Burghley · CCI5*-L",
            level="CCI5*-L",
        ...
        results = parse_leaderboard_results(BURGHLEY_LIVE_THURSDAY_AFTERNOON_HTML, board=board)
        self.assertEqual(len(results), 5)
        ordered = sorted(results, key=lambda result: result.finishing_score)
        self.assertEqual(
            [(result.horse_name, result.finishing_score) for result in ordered],
        ...
        self.assertEqual(leader.show_jumping_penalties, 0.0)
        bragg = next(result for result in results if result.horse_name == "Jaeger Master")
        self.assertEqual(bragg.rider_name, "Alexander BRAGG (GBR)")
        self.assertEqual(bragg.dressage_score, 35.6)
        mason = next(result for result in results if result.horse_name == "Centennial")
    ...
    def test_burghley_friday_first_ride_adds_watson(self):
        board = RechenstelleBoard(
            url="https://live.rechenstelle.de/2026/burghley/leaderboard01.html",
            event_name="Burghley · CCI5*-L",
            level="CCI5*-L",
        ...
        results = parse_leaderboard_results(BURGHLEY_LIVE_FRIDAY_FIRST_RIDE_HTML, board=board)
        self.assertEqual(len(results), 3)
        ordered = sorted(results, key=lambda result: result.finishing_score)
        self.assertEqual(
            [(result.horse_name, result.finishing_score) for result in ordered],
    ...
    def test_burghley_friday_second_ride_adds_jonelle_price(self):
        board = RechenstelleBoard(
            url="https://live.rechenstelle.de/2026/burghley/leaderboard01.html",
            event_name="Burghley · CCI5*-L",
            level="CCI5*-L",
        ...
        results = parse_leaderboard_results(BURGHLEY_LIVE_FRIDAY_SECOND_RIDE_HTML, board=board)
        self.assertEqual(len(results), 3)
        ordered = sorted(results, key=lambda result: result.finishing_score)
        self.assertEqual(
            [(result.horse_name, result.finishing_score) for result in ordered],
            [
                ("Toblerone", 29.5),
                ("Chilli's Midnight Star", 36.2),
                ("Ballyneety Rocketman", 40.5),
            ],
        )
        price = next(result for result in results if result.horse_name == "Chilli's Midnight Star")
        self.assertEqual(price.rider_name, "Jonelle PRICE (NZL)")
        self.assertEqual(price.dressage_score, 36.2)
        self.assertEqual(price.show_jumping_penalties, 0.0)
        watson = next(result for result in results if result.horse_name == "Ballyneety Rocketman")
        self.assertEqual(watson.dressage_score, 40.5)
    ...
    def test_burghley_friday_evening_corrects_meade_and_skips_mason_wdbxc(self):
        board = RechenstelleBoard(
            url="https://live.rechenstelle.de/2026/burghley/leaderboard01.html",
            event_name="Burghley · CCI5*-L",
            level="CCI5*-L",
        ...
        self.assertEqual(by_horse["Falco"].dressage_score, 30.5)
        self.assertEqual(by_horse["Cavalier Crystal"].rider_name, "Harry MEADE (GBR)")
        self.assertEqual(by_horse["Cavalier Crystal"].dressage_score, 38.4)
        self.assertEqual(by_horse["Bally Louis"].rider_name, "Holly RICHARDSON (GBR)")
        self.assertEqual(by_horse["Bally Louis"].dressage_score, 40.7)
        self.assertEqual(by_horse["Cavalier Crystal"].show_jumping_penalties, 0.0)
        self.assertEqual(by_horse["Cavalier Crystal"].cross_country_jump_penalties, 0.0)
        self.assertEqual(by_horse["Cavalier Crystal"].cross_country_time_penalties, 0.0)
        self.assertNotIn("Centennial", {result.horse_name for result in results})

    def test_burghley_final_results_keep_finishers_and_skip_status_rows(self):
        board = RechenstelleBoard(
            url="https://live.rechenstelle.de/2026/burghley/leaderboard01.html",
            event_name="Burghley · CCI5*-L",
            level="CCI5*-L",
        ...
        self.assertEqual(
            [(result.horse_name, result.finishing_score) for result in ordered],
            [("Chilli Knight", 29.6), ("Treworra", 43.0), ("HSH Blake", 44.0)],
        )
        winner = ordered[0]
        self.assertEqual(winner.rider_name, "Gemma STEVENS (GBR)")
        self.assertEqual(winner.dressage_score, 29.6)
        self.assertEqual(winner.show_jumping_penalties, 0.0)
        self.assertEqual(winner.cross_country_jump_penalties, 0.0)
        self.assertEqual(winner.cross_country_time_penalties, 0.0)
        magee = ordered[1]
        self.assertEqual(magee.rider_name, "Katie MAGEE (GBR)")
        self.assertEqual(magee.dressage_score, 36.2)
        self.assertEqual(magee.show_jumping_penalties, 4.4)
        self.assertEqual(magee.cross_country_jump_penalties, 2.4)
        self.assertEqual(magee.cross_country_time_penalties, 0.0)
        horses = {result.horse_name for result in results}
        self.assertNotIn("Falco", horses)
