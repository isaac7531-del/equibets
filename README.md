...
2. National-event sources fill gaps after FEI data for every FEI member nation
   and every domestic eventing level.
3. Region-specific national sources can still be prioritized where known, while
   `global_national_federations` is the backfill path for every country and
   level not covered by a more specific source.
