...
2. National-event sources fill gaps after FEI data across all eventing levels,
   ...
   USA.
3. `global_national_federations` is the all-country, all-level backfill path for
   national events after the priority regions are covered.

Use `sources_for_region(region, level=...)` for regional coverage and
`sources_for_country(country, level=...)` when selecting sources for a specific
country and eventing level.
