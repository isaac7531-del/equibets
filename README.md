...
`equibets.sources`. Registry version 2 declares national-event coverage for
`all_countries` and `all_eventing_levels`; source-selection helpers can filter
by region, country, and optional level.

1. `data_fei` (`https://data.fei.org/`) is the primary source for international
   eventing results across all countries.
2. National-event sources fill gaps after FEI data at every eventing level,
...
3. `global_national_federations` is the backfill path for national events from
   every country and level after the priority regions are covered.
