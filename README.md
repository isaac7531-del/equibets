...
2. Country and regional national-event sources fill gaps after FEI data when a
   direct federation connector is available.
3. `global_national_federations` is the backfill path for national events from
   every FEI member nation across all national levels.
