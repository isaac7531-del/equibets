...
3. National-event coverage uses `all_eventing_levels` for international,
   national, regional, local, and introductory levels.
4. `global_national_federations` is the `all_countries` backfill path for
   national events from every country after the priority regions are covered.
5. Use `sources_for_country(country, level=...)` when selecting registry entries
   for a specific country and competition level.
