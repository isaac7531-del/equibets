...
1. `data_fei` (`https://data.fei.org/`) is the primary source for eventing
   results across all countries when the result appears in FEI data.
2. National-event sources fill gaps after FEI data, covering all eventing levels
   with priority coverage for Europe, the UK, Australia, New Zealand, and the USA.
