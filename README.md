...
2. National-event sources fill gaps after FEI data for every country and all
   event levels, including national championship, national, regional, local,
   schooling, training, and unaffiliated starts.
3. `global_national_federations` is the backfill path for national events from
   every country, while country-specific federation sources can add richer
