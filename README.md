...
The source registry supports `all_countries` and `all_eventing_levels`
wildcards. Use `sources_for_country(country, level=...)` when resolving sources
for a specific national event, or `sources_for_region(region, level=...)` when
working from a broader regional queue.
