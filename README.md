...
- Tracks public event-results sources for FEI and all-country national-event coverage.
...
`equibets.sources`. It can be queried by priority region or by country code and
optional eventing level.

1. `data_fei` (`https://data.fei.org/`) is the primary source for eventing
   results across all FEI member nations.
2. National-event sources fill gaps after FEI data, with all-level priority
   coverage for Europe, the UK, Australia, New Zealand, and the USA.
...
4. Use `sources_for_country("USA", level="training")` when selecting national
   sources for a specific country and level.
