...
3. `global_national_federations` is the all-country, all-eventing-level
   backfill path for national events after the priority regions are covered.
4. Use `sources_for_country(country, level=...)` or
   `sources_for_region(region, level=...)` to resolve source coverage for a
   specific country or eventing level.
