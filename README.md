...
3. `global_national_federations` is the backfill path for national events from
   ...
   covered. Registry wildcards use `all_countries` and `all_eventing_levels`.

Country and level-aware source lookup is available from Python:

```python
from equibets.sources import sources_for_country

sources = sources_for_country("BRA", level="Beginner Novice")
```
