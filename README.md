...
1. `data_fei` (`https://data.fei.org/`) is the primary source for eventing
   results across all configured FEI country codes.
2. National-event sources fill gaps after FEI data across every configured
   national level: championship, national, regional, state/provincial, local,
   club, schooling/training, youth/pony club, and open unaffiliated events.
3. `global_national_federations` is the backfill path for national events from
   every configured country code after the priority regions are covered.
