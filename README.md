...
2. National-event sources fill gaps after FEI data across the configured
   domestic level taxonomy: national championships, national, regional,
   state/provincial, area, local, grassroots, schooling, training,
   introductory, youth, pony, and amateur levels.
...
4. `global_national_federations` is the backfill path for national events from
   all countries after the priority regions are covered.
