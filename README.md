...
2. National-event sources fill gaps after FEI data, with priority coverage for
   Europe, the UK, Australia, New Zealand, and the USA at every eventing level.
3. `global_national_federations` is the backfill path for national events from
   ...
   covered.
4. `equibets.sources` can filter configured sources by region, country, and
   eventing level while preserving source priority.
