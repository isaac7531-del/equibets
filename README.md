...
1. `data_fei` (`https://data.fei.org/`) is the primary source for eventing
   results across all countries when a result is available there.
2. National-event sources fill gaps after FEI data at every eventing level, with
...
Use `sources_for_country(country, level=...)` to resolve country-level coverage
or `sources_for_region(region, level=...)` for region-level source discovery.
