...
2. National-event sources fill gaps after FEI data, with priority coverage for
   Europe, the UK, Australia, New Zealand, and the USA across every eventing
   level.
3. `global_national_federations` is the `all_countries` / `all_eventing_levels`
   backfill path for national events after the priority regions are covered.
4. Use `sources_for_country(country, level=...)` when selecting source coverage
   for a specific country and eventing level.
