...
1. `data_fei` (`https://data.fei.org/`) is the primary source for eventing
   results across every country with published FEI data.
2. National-event sources fill gaps after FEI data, with all-level coverage for
...
4. Use `sources_for_country(country, level=...)` to find source coverage for a
   specific country and eventing level.
