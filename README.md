...
2. National-event sources fill gaps after FEI data across every available
   national-event level, with priority coverage for Europe, the UK, Australia,
   New Zealand, and the USA.
3. `global_national_federations` is the backfill path for national events from
   every FEI member nation and every available national-event level after the
