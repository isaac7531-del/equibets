...
2. National-event sources fill gaps after FEI data, with coverage for every
   country and every published national level.
3. `global_national_federations` is the backfill path for national events from
   every country after higher-priority national sources are covered.
