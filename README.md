...
`equibets.sources`. It defines the all-country coverage target
(`all_fei_member_nations`), concrete national/regional event-level IDs, and FEI
international level IDs used by the weekly update flow.
...
3. `global_national_federations` is the backfill path for national events from
   every FEI member nation and every configured national/regional level after
   the priority regions are covered.
