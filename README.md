...
2. National-event sources fill gaps after FEI data for every FEI member nation
   and every national level, from championships through local, grassroots,
   introductory, amateur, youth, pony, and schooling results.
3. Specific national and regional federation sources are preferred where they
   exist; `global_national_federations` is the backfill path when no
   country-specific source is configured.
