...
1. `data_fei` (`https://data.fei.org/`) is the primary source for eventing
   results across all countries and eventing levels when those results are
   available in FEI data.
2. National-event sources fill gaps after FEI data for every country and every
   eventing level, with priority coverage for Europe, the UK, Australia, New
   Zealand, and the USA.
