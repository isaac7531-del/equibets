...
1. `data_fei` (`https://data.fei.org/`) is the primary source for international
   eventing results across all FEI member nations.
...
3. Regional national-federation registries cover Africa, Asia, Central America
   and the Caribbean, Europe, the Middle East, North America, Oceania, and South
   America.
4. `global_national_federations` is the backfill path for national events from
   every FEI member nation after the priority regions and regional registries
   are covered.

The registry declares coverage targets for every FEI member nation and every
configured national level from `starter` through `national_five_star`. Helpers
such as `sources_for_country()` and `sources_for_event_level()` preserve source
priority while filtering the registry for collectors.
