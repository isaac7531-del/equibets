...
- Track all FEI member nations over time, while prioritizing richer
  national-event data from regional federation registries and higher-priority
  UK, Australia, New Zealand, and USA sources.
...
1. Pull new FEI international-level results from `data.fei.org`.
2. Pull national-event updates from regional federation registries.
3. Pull higher-priority national feeds for the UK, Australia, New Zealand, and
   the USA.
4. Pull global national-federation results as an all-country, all-domestic-level
   backfill.
