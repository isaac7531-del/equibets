...
- Track all countries over time with national-event data from every supported
  event level, while using country-specific federation sources where available.
...
2. Pull national-event updates from country-specific federation sources.
3. Pull global national-federation and organizer results as a backfill for
   every country and level.
