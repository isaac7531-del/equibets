...
- Track national-event data for every FEI member nation and every national level,
  while prioritizing richer connectors for Europe, the UK, Australia, New
  Zealand, and the USA when duplicates exist.
...
2. Pull national-event updates for every country and every national level exposed
   by configured source connectors.
3. Pull global national-federation results as the all-country/all-level backfill.
