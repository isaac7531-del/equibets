...
Country-specific national-event refreshes should use
`sources_for_country(country, level=...)` so priority national sources and the
global all-country/all-level backfill are selected consistently.
