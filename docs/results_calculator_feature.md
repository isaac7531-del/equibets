...
- Track all nations over time, while prioritizing richer direct national-event
  data where configured and using regional/global federation registries for the
  rest of the FEI member nations.
...
2. Pull direct national-event updates where a country-specific source is
   configured.
3. Pull regional and global national-federation results as a backfill for every
   FEI member nation and every published national level.
