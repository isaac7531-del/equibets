...
- Track all FEI member nations over time, with national-event data scoped to
  every normalized level from championships through club, grassroots,
  schooling/training, and youth divisions.
...
2. Pull national-event updates from the high-priority direct sources.
3. Pull national-event updates from the regional federation registries.
4. Pull global national-federation results as a backfill for every FEI member
   nation and every normalized national level.
