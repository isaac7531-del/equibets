...
- Track all nations over time, while collecting national-event data across all
  domestic eventing levels.
...
2. Pull regional national-federation updates for every configured global region.
3. Pull country-specific national-event sources where configured.
4. Pull global national-federation results as a backfill for every FEI member
   nation and domestic eventing level.
