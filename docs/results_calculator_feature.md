...
- Track all FEI member nations over time, using national-event data from every
  available level while prioritizing richer sources from Europe, the UK,
...
2. Pull national-event updates from the priority regions across every available
   national-event level.
3. Pull global national-federation results as a backfill for every FEI member
   nation and every available national-event level.
