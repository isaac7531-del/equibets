...
- Track all FEI member nations and eventing levels over time, while
  prioritizing richer national-event data from regional federation backfills
  and country-specific sources for the UK, Australia, New Zealand, and the USA.
...
1. Pull new FEI results from `data.fei.org` for all declared FEI international
   and championship levels.
2. Pull national-event updates from the priority regional and country sources
   for all declared domestic levels.
3. Pull global national-federation results as a backfill for every FEI member
   nation.
