...
- Track national-event data from every country and every eventing level over
  time, while prioritizing richer feeds from Europe, the UK, Australia, New
  Zealand, and the USA.
...
2. Pull national-event updates from the priority regions at every eventing
   level.
3. Pull every-country national-federation results as a backfill.
