...
- Track every country over time with national-event data from every eventing
  level, while keeping country-specific federation sources where they provide
  richer coverage.
...
2. Pull country-specific national-event updates where federation sources are
   configured.
3. Pull global national-federation and event-aggregator results as the
   all-country, all-level backfill.
