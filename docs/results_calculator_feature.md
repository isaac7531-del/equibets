...
- Track national-event data from every country and every eventing level over
  time, while prioritizing richer national-event data from Europe, the UK,
...
2. Pull national-event updates from the priority regions across all national
   eventing levels.
