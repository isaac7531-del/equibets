...
- Track every configured FEI country/NF code and eventing level over time, while
...
3. Pull global national-federation results as an all-country/all-national-level
   backfill.
...
`national`, `Beginner Novice`, or `CCI5*-L`. This makes all-country and
all-level coverage explicit while individual federation crawlers are added
incrementally.
