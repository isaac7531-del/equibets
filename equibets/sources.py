...
def _covers_level(source: EventSource, level: str | None) -> bool:
    ...
    levels = {_normalize_key(item) for item in source.event_levels}
    wildcards = {_normalize_key(wildcard) for wildcard in LEVEL_WILDCARDS}
    return bool(levels & wildcards) or _normalize_key(level) in levels


def _normalize_country(country: str) -> str:
    return _normalize_key(country).upper()
