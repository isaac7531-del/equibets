...
def sources_for_region(
    ...
    *,
    ...
    return [
        ...
        and _covers_event_level(source, level)
    ]
...
def _sources_with_status(path: Path | str, include_planned: bool) -> list[EventSource]:
    statuses = {"active", "planned"} if include_planned else {"active"}
    return [source for source in load_event_sources(path) if source.status in statuses]


def _covers_region(source: EventSource, normalized_region: str) -> bool:
    regions = {_normalize_token(region) for region in source.regions}
    return "global" in regions or normalized_region in regions
...
def _covers_country(source: EventSource, normalized_country: str) -> bool:
    ...
    return bool(COUNTRY_WILDCARDS & countries) or normalized_country in countries


def _covers_event_level(source: EventSource, level: str | None) -> bool:
    if level is None:
        return True

    levels = {_normalize_token(event_level) for event_level in source.event_levels}
    return bool(EVENT_LEVEL_WILDCARDS & levels) or _normalize_token(level) in levels
