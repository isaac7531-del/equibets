...
def expand_event_levels(
    ...
    *,
    ...
    """Expand registry level groups into concrete source-level tags."""

    return _expand_event_levels(event_levels, load_event_level_groups(coverage_path))


def _expand_event_levels(
    event_levels: Iterable[str],
    groups: Mapping[str, tuple[str, ...]],
) -> tuple[str, ...]:
    """Expand level groups with already-loaded coverage metadata."""

    expanded: list[str] = []
    seen: set[str] = set()
    for event_level in event_levels:
        for expanded_level in groups.get(event_level, (event_level,)):
            if expanded_level not in seen:
                expanded.append(expanded_level)
...
def sources_for_event_level(
    ...
    *,
    ...
    statuses = {"active", "planned"} if include_planned else {"active"}
    event_level_groups = load_event_level_groups(coverage_path)
    ...
    return [
        ...
        in {
            ...
            for level in _expand_event_levels(source.event_levels, event_level_groups)
        }
    ]
